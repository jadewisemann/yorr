...
- **방 폐쇄 스케줄러**(인메모리, 단일 스레드): 방당 예약 1개, 재예약은 교체.
  ...
  의도적 이중 방어다.
- **부팅 재무장**(`game/startupResume.ts`): SCAN으로 모든 방을 훑어 **PLAYING 방만**
  본다. 방마다 그 게임 모듈의 `rehydrate`를 불러 **저장된 마감으로 판을 이어가고**,
  이어갈 수 없는 방만 close한다. LOBBY·FINISHED는 건드리지 않는다(배포마다 로비가
  날아가던 회귀의 재발 방지).
  - **`StaleRoomCleaner`를 대체했다.** 예전에는 마감 타이머가 인메모리라 재무장 경로가
    없었고, 그래서 PLAYING 방을 **전부** close하는 것이 정책이었다. 마감 시각이 Redis로
    나간 뒤(PLAN.md PR 6) 되살리는 쪽이 가능해졌다.
  - 실패는 **방 단위로 격리한다**: 한 방의 실패가 나머지 방의 복구를 막으면 예전보다
    나빠지고, 되살리지 못한 방을 열어 둔 채 넘어가면 상태만 살아 있고 턴이 멈춘 채
    JOIN도 `game_started`로 거부하는 최악의 상태가 남는다. 그래서 실패한 방만 닫는다.
  - 되살리는 경로에서는 **오프라인 판정을 하지 않는다.** 부팅 시점에는 아직 아무 소켓도
    붙지 않아 레지스트리가 전원을 오프라인으로 답하므로, 그 판정을 태우면 재무장이 곧
    턴 스킵이 되고 두 턴이면 재시작만으로 사람이 방에서 쫓겨난다.
