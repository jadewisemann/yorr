...
게임 하나 = 모듈 하나(`src/game/module.ts`).
...
**`name`·`minPlayers`·`maxPlayers`·`supportsBots`는 모듈에 없다.** 모듈이
...
세 게임 모두 시작 시 **사람만 골라 호스트를 맨 앞에** 세운다(`game/startRoster.ts`).
그 순서가 곧 턴 순서이고 호스트가 선이다. 봇을 지원하는 게임(야추)은 자기 좌석 배치를
따로 갖고 있으므로 이 헬퍼를 쓰지 않는다.
...
- 모듈이 `handle`에서 던지면 잡지 않는다 — 게이트웨이가 로그만 남기고 소켓을
  살려 둔다.
  **응답은 모듈이 스스로 보낸다.**
- 아웃바운드 타입 조립: `game.<code소문자>.<event>` (`gameWsType`).
...
- `start`: 활성 플레이어가 오프라인이면(봇은 절대 오프라인 아님) 타이머 없이
  ...
  방 스냅샷은 **한 번만** 읽어 오프라인 판정과 아래 연습 방 판정에 함께 쓴다.
- **연습 방에는 시계가 없다**(`UNTIMED_HUMAN_LIMIT = 1`). 봇을 뺀 사람이 하나
  이하인 방은 마감을 걸지 않고 `deadline: null`을 방송한다 — 턴 제한의 목적이
  "멈춘 한 사람 때문에 나머지가 기다리는 것"을 막는 것인데 기다리는 사람이 없기
  때문이다. 방 스냅샷을 못 읽었으면(null) 판단하지 않고 **기존 동작(시계 있음)**
  으로 떨어진다.
  - 예외 하나: 연습 방이라도 **봇 턴에는 예약을 남긴다**(화면에는 여전히 시계가
    없다). 봇 스텝의 예외는 삼켜지고 라운드 타이머가 유일한 폴백이라
    ([yacht.md 「실패 격리」](games/yacht.md)), 예약까지 지우면 굴림 한 번 실패한
    연습 방을 아무도 깨우지 못한다.
  - `currentDeadline`은 그래서 **세 값**이다: 시각 / `null`(시계 없는 턴) /
    `undefined`(진행 중인 턴 없음). 재접속은 `undefined`만 실패로 본다.
...
- **바깥 계층은 전부 좁은 포트로 뒤집었다**(`roundPorts.ts`): 브로드캐스터·
  ...
  조회. 구체 타입 여섯을 직접 잡지 않는 이유는 둘: 라운드
...
- 아웃바운드 타입은 주입된 `gameCode`(기본은 `catalog.ts`의 `YACHT_DICE`)와
  `module.ts`의 `gameWsType`으로 조립한다 — 타이머가 야추
...
- 결과 타입은 한 레코드에 두 필드를 두고 하나만 채우는 모양 대신 **판별
  유니온**이다. 정적 팩터리로 지켜야 했을 "kind를 보고
  꺼내라"는 규약이 타입으로 강제된다.
...
- 완료된 게임(`finished`)은 STALE로 즉시 접는다. 여기서 한 번 더 제출을
...
- 별도의 상수 이름 체계를 **두지 않는다** — 와이어·Redis 필드·조회
...
- 키 순서는 `SCORE_CATEGORIES`(ones…yacht) 그대로다. 응답 필드 이름은
