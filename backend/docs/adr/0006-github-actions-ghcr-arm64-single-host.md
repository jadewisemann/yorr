...
> **갱신 메모 (2026-08-22) — Always Free A1 한도가 절반으로 줄었고, 인스턴스를 잃는
> 시나리오가 실재한다.**
>
> 한도가 4 OCPU / 24GB에서 **2 OCPU / 12GB**로 축소됐다(2026-06-15 발효,
> 2026-08-18부터 공지 없이 집행). 위 표의 숫자는 그 뒤의 값이라 그대로 맞다.
> 바뀌는 것은 두 가지다.
>
> - **"남는 자원으로 별도 인스턴스에 Jenkins·Grafana" 안이 닫혔다.** 아래 기각
>   목록의 "OCI에 Jenkins를 세운다"가 자원 여유를 이유로 다시 열릴 일은 없다.
> - **인스턴스가 종료될 수 있다.** 공인 IP가 **ephemeral**이므로 인스턴스를 잃으면
>   주소도 함께 잃는다. 그 주소는 `deploy/.env`·`frontend/vercel.json`·Vercel
>   환경변수 **세 곳**에 박혀 있고 뒤 둘이 프론트라 재배포가 따라온다
>   ([`deploy/PLAN.md`](../../../deploy/PLAN.md) §12).
>
> 한편 위 「빌드 위치」의 판단 자체는 유효하지만 근거의 크기는 과대평가였다.
> `backend/`를 실제로 빌드해 보면 verify 전체가 약 24초 / 61.5 CPU-초이고 peak RSS는
> 360 MiB다. 빌드가 만드는 스케줄러 지터는 3~50ms인데 라운드 마감 유예가
> 1,000ms이므로 유예 예산의 0.3~5%다. 이 결정을 실제로 지탱하는 것은 CPU 경합이
> 아니라 아래 기각 목록의 ②"빌드 실패 시 배포 가능한 이미지가 없다"와
> ③"이미지 태그의 재현성"이다.
...
- ~~**배포는 진행 중인 게임을 끊는다.**~~ **2026-08-23 갱신:** 이 전제는 더 이상
  성립하지 않는다. 마감 시각이 Redis로 나가고 부팅 재무장이 생겼다
  (`game/startupResume.ts`, [`deploy/PLAN.md`](../../../deploy/PLAN.md) PR 6).
  재시작하면 소켓은 여전히 전부 끊기지만 **판은 이어진다** — 되살릴 수 없는 방만
  닫힌다. 아래 "시각 선택이 유일한 완화책"이라는 결론도 그만큼 약해졌다.
...
> **갱신 메모 (2026-08-21) — "사람이 시작한다"를 "게임이 없을 때 시작한다"로 바꿨다.**
>
> 위 결정의 근거는 두 개였고, 둘 다 남아 있다: ① 배포가 진행 중 게임을 끊는다
> ② 배포 키를 GitHub Secrets에 두면 저장소 쓰기 권한이 곧 운영 셸 권한이 된다.
> 바꾼 것은 **"시각 선택"을 사람의 눈대중이 아니라 측정에 맡긴다**는 점뿐이다.
>
> `deploy/auto-deploy.sh`(systemd timer, 5분)가 호스트에서 돌며 ① `git pull`
> ② 이미지 pull ③ 바뀐 것이 있는지 ④ `yorr_rooms_active`(PLAYING 방 수)가 0인지를
> 보고, 넷이 맞을 때만 손 배포와 **같은 경로**(`deploy.sh -y`)로 배포한다.
>
> - ②의 근거는 그대로 지켜진다 — **호스트가 당기고, 아무도 밀지 않는다.** GitHub에
>   키가 없고 22번 포트를 러너에게 열지 않는다(여는 것은 여전히 80·443뿐).
> - ①은 없어지지 않았다. 게임 중이면 미루고 다음 회차에 다시 본다. 다만 바쁜 서버에서
>   영원히 밀리지 않도록 상한(`YORR_DEPLOY_MAX_DEFER`, 기본 6시간)을 두었고, 그
>   상한에 걸린 배포는 게임을 끊는다 — 이 방식이 없애는 것은 "사람이 골라야 한다"이지
>   "게임이 끊긴다"가 아니다.
> - 게이지를 읽지 못하면(`unknown`) **0으로 보지 않는다.** 못 읽었을 때 끊는 쪽으로
>   기울지 않는 것이 기본값이고, 그때도 상한이 최후의 출구다.
> - 타이머를 끄면(`systemctl disable --now yorr-auto-deploy.timer`) 이 ADR의 원래
>   상태로 정확히 돌아간다. 손 배포 경로는 그대로 남아 있다.
>
> **버튼도 붙였다(`.github/workflows/deploy.yml`, `workflow_dispatch`).** 기각했던
> 것은 "버튼"이 아니라 **워크플로가 SSH로 미는 방식**이었다. 셀프호스티드 러너는
> 호스트에서 GitHub으로 **나가는** 연결로 일감을 받으므로 22번 포트를 열지 않고
> 배포 키를 Secrets에 두지 않는다 — ②의 근거가 그대로 지켜진다. 새로 생긴 비용은
> 두 가지다: 호스트에 러너 서비스 하나가 상주한다(2 OCPU를 나눠 쓰지만 배포
> 순간에만 일한다), 그리고 **셀프호스티드 러너에서 믿을 수 없는 코드를 돌리면
> 곧 호스트 셸이다** — 그래서 이 러너를 쓰는 워크플로는 `workflow_dispatch`
> 하나뿐이고 `pull_request`로는 돌지 않는다(나머지는 `ubuntu-latest`).
> 버튼은 ①을 지키지 않는다(누르면 게임을 끊는다) — 끊고 싶지 않으면 누르지 않고
> 타이머에 맡긴다. 롤백은 `.env`의 `BACKEND_IMAGE` 고정으로 이뤄지며(위 롤백 절과
> 같은 메커니즘) 고정된 동안 자동 배포는 정지한다.

> **갱신 메모 (2026-08-22) — 셀프호스티드 러너를 제거하기로 했다. 버튼 경로가 사라진다.**
>
> 바로 위 메모가 버튼을 붙이며 든 논거("나가는 연결이므로 22번 포트를 열지 않고
> 배포 키를 Secrets에 두지 않는다")는 **여전히 옳다.** 그것은 SSH 배포와 비교했을
> 때의 이야기이고, 바뀐 판단은 **러너를 아예 두지 않는 쪽이 더 낫다**는 것이다.
>
> 근거 셋:
>
> - **`deploy.yml` 실행 횟수가 0회다.** 2026-08-21 등록 이후 한 번도 돌지 않았다.
>   실익 없는 상주 컴포넌트다.
> - **저장소가 public이고 러너 계정이 docker 소켓을 쥐고 있다** — 워크플로 실행이
>   사실상 호스트 root다. GitHub 공식 보안 가이드도 public 저장소의 셀프호스티드
>   러너를 특히 경계한다. 위 메모가 "새로 생긴 비용"으로 적어 둔 바로 그 항목이며,
>   `workflow_dispatch` 하나로 좁혀도 공격면 자체는 남는다.
> - **같은 pull 기반 배포를 systemd 타이머가 이미 한다.** 경로가 둘이면 한쪽이 낡는다.
>
> 최종 신뢰 방향은 GitHub → GHCR ← OCI(pull only)다. GHCR 패키지가 public이라
> **OCI가 GitHub 자격증명을 하나도 들지 않는다.** SSH는 남기되 CI/CD control plane이
> 아니라 장애 조사·수동 롤백·bootstrap·OS 유지보수용 break-glass 경로로만 쓴다.
>
> 함께 바뀌는 것: 배포 단위를 이미지 하나가 아니라 **"git revision + compose/설정 +
> image digest"를 묶은 Release**로 정의하고, 롤백도 그 전체를 되돌린다. `:main`은
> 발견용 포인터일 뿐이고 실행은 digest로 고정한다. 계획과 근거는
> [`deploy/PLAN.md`](../../../deploy/PLAN.md)에 있으며 **아직 구현하지 않았다** —
> 이 ADR의 본문은 호스트 cutover가 끝난 뒤에 새 ADR로 대체한다.
...
1. **구 백엔드 배포 경로.** 전환이 끝나기 전까지 구 호스트의 백엔드가
...
건드리지 않는다. 삭제는 전환의 마지막 항목("구 백엔드 제거 +
낡은 문서 정리 — 별도 PR")에서, 프론트 배포를 GitHub Actions나 Vercel Git 연동으로
옮긴 뒤에 한다.

> **갱신(2026-08-21): 삭제했다.** 위 두 전제가 둘 다 사라졌다 — 프론트 배포는 이미
> Vercel로 옮겨졌고(Jenkins 잡이 돌지 않는다), 구 백엔드 배포 스테이지는
...
- **GitHub의 ARM 러너(`ubuntu-24.04-arm`)로 네이티브 빌드.** 가장 직관적이고
  ...
  **arm64에서 실행할 것이 아무것도 없다** — 러너 아키텍처를 바꿔 얻을 것이 없다.
  > **갱신 (2026-08-22): 첫 번째 기각 근거는 무효다.** 저장소가 **public**이므로
  > Actions 분이 무제한이고 `ubuntu-24.04-arm`도 무료다.
  > **그래도 동작은 바꾸지 않는다.** 두 번째 근거가 그대로 살아 있고, arm64
  > build-push가 이미 26초(main 파이프라인 전체 2분 52초)라 얻을 속도가 없다.
  > 오히려 네이티브 러너로 옮기면 `prod-deps` 스테이지의 `*.node` 가드가 지키던
  > 규율("런타임 스테이지에 `RUN`을 넣지 않는다")이 조용히 사라진다. 고칠 것은
  > 이 한 줄의 근거 서술이지 워크플로가 아니다.
...
- **`main` push마다 자동 배포**(워크플로에서 SSH). 편하다. 기각: 위 3번 —
  ...
  - 2026-08-21: **push가 아니라 pull로 자동화했다**(위 3번의 갱신 메모). 기각
    사유 중 살아남은 것은 "워크플로에서 SSH로 민다"는 방향이었고, 호스트가 당기면
    키도 열린 포트도 필요 없다. "게임을 끊는다"는 쪽은 `yorr_rooms_active` 게이트로
    다룬다 — 기각을 뒤집은 것이 아니라 **다른 축**을 찾은 것이다.
...
- **빈 MySQL 위에서는 backend가 뜨지 못한다.** `verifyMigrations`는 읽기 전용이고
  ...
  **진입점이 없다**(npm 스크립트도 CLI도 없고 테스트만 부른다). 새 호스트의 첫
  ...
  - 새 호스트의 MySQL은 구 백엔드와 공유하지 않으므로 ADR-0005의 "전환기
    스키마 동결"이 여기를 묶지 않는다.
