...
/**
 * 야추 모듈이 **자기 바깥**에 요구하는 것들 — 전부 이 파일의 좁은 포트로만
 * 표현한다.
 *
 * 왜 포트인가(`round/roundPorts.ts`·`reconnect/reconnectPorts.ts`와 같은 이유):
 ...
 *
 * **레지스트리 phase를 옮기는 것은 게임 모듈의 일**이다. 이게 빠지면 REST로
...
/**
 * `GameReconnectSnapshotService.snapshot`의 자리.
 *
 ...
 *
 * 타이머 구현이 `requestMsgId`에 기본값(`null`)을 갖고 있어도 이 시그니처를 만족한다.
 */
export interface YachtRoundTimer {
  ...
  /** 재접속 복귀 시 오프라인 결석 횟수 리셋. **스냅샷 조립 뒤에** 부른다(재접속의 순서 계약). */
  clearOfflineMisses(roomId: string, playerId: string): void
}

/**
 * `ScoreRoundSubmissionService<RoundSubmissionResult>`의 자리.
 *
