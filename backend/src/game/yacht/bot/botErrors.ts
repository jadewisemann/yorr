 ...
 * 폴백 전략으로 내려가고(Java의 `catch (RuntimeException)`), 오케스트레이터가
...
/** Java `IllegalArgumentException`·`IllegalStateException` 자리. */
 ...
 *
 * Java에는 없는 오류다(Java는 스레드 풀 위에서 무한정 돌 수 있었다). Node는
 * 단일 스레드라 탐색이 이벤트 루프를 물고 있는 동안 **다른 방의 WS·하트비트·
