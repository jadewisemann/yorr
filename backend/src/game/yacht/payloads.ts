 ...
 * 인바운드 payload 파싱.
 *
 * **와이어 계약이 관용을 요구한다**: 타입이 어긋나면 파싱 실패(→ `INVALID_MESSAGE`
 * "invalid dice.roll payload"), **없는 필드는 기본값**(숫자 0, 참조 null)이 된다.
...
/** `direction`은 null로 남긴다 — 이 payload는 null 필드도 와이어에 그대로 실리는 계약이다. */
