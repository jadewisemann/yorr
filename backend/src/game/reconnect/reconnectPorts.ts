/**
 * 재접속 스냅샷·고아 상태 스윕이 **자기 바깥**에 요구하는 것들 — 전부 이
 * 파일의 좁은 포트로만 표현한다.
 *
 * 왜 포트인가: 구체 타입(`RealtimeRoomSnapshotService`·`RoundSynchronizationService`·
 * `RoundTimerService`·`GameScoreQueryService`·`RoomService`)을 직접 잡으면 안 되는
 * 이유는 `round/roundPorts.ts`가
 ...
 * (`activePlayerId`는 getter라 구조적으로 프로퍼티다).
 *
 ...
 *
 * `GameScoreQueryService.getScoreboards`는 `ReadonlyMap`을 돌려준다
 * (playerId 오름차순을 스토어가 정한 순서 그대로 보존하려고). 와이어에는 JSON
 * 객체가 실려야 하는데 **JS의 `Map`은 `JSON.stringify`가 `{}`로 만든다** —
 * 그대로 스냅샷에 실으면 점수판이 통째로
 ...
 * 보낼 뿐이라 `ScoreBoard` 도메인 타입에 묶일 이유가 없다(`ConfirmedScore.
