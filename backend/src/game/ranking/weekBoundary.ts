 ...
 *
 * 주간 랭킹의 유일하게 미묘한 지점이다. 경계가 어긋나면 주가 바뀌는 순간에만
 * 틀리므로 굴려서는 잡히지 않는다(테스트가 초 단위로 못박아 둔 이유).
 *
 * ## 왜 존을 코드에 고정하는가
 * KST 오프셋을 상수로 박는다. 인프라의 `TZ` 환경변수에 맡기면 개발 컨테이너(Asia/Seoul)와 운영(UTC)이 서로 다른 주를 세게 된다 —
...
export interface WeekBoundary {
  /**
   * 이 주가 시작하는 **KST 달력 날짜**(`YYYY-MM-DD`) — 프론트의 "이번 주" 표기를 서버 기준으로 맞추려면
