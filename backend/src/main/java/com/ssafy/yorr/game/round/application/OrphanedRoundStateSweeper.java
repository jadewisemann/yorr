...
/**
 * 방이 사라졌는데도 남아 있는 인메모리 라운드 상태를 주기적으로 걷어낸다.
 * <p>
 * <b>왜 필요한가:</b> Redis 키는 TTL로 스스로 사라지지만 TTL은 서버 메모리를 청소해주지 않는다.
 * 인메모리 자원({@code InMemoryRoundStateStore} · 마감 타이머 · 오프라인 결석 카운트)을 회수하는
 * 경로는 빈 방 유예 타이머 하나뿐인데, 그 예약은 프로세스가 재시작되면 사라진다. 그러면 그
 * 라운드 상태는 아무도 치우지 않는다.
 * <p>
...
public class OrphanedRoundStateSweeper {
     ...
     * 짧게 잡을 이유도 없다(한 항목이 수 KB 수준이라 몇 분 더 남아도 무해하다).
     */
