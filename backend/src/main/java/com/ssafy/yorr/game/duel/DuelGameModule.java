...
@Component
public class DuelGameModule implements GameModule {

    public static final String CODE = "DUEL";

    private final DuelGameService games;
    private final RoomSessionRegistry sessions;
    private final ObjectMapper objectMapper;

    public DuelGameModule(
            DuelGameService games,
    ...
    public boolean handles(String messageType) {
        return "draw".equals(messageType);
    }
    ...
    public void handle(WebSocketSession session, InboundEnvelope message) throws IOException {
        ...
        try {
            DuelDrawPayload payload = objectMapper.treeToValue(message.payload(), DuelDrawPayload.class);
            games.draw(message.roomId(), member.playerId(), payload);
        } catch (IllegalArgumentException exception) {
            sendError(session, WsErrorCode.INVALID_MESSAGE, exception.getMessage(), message.msgId());
        } catch (Exception exception) {
            sendError(session, WsErrorCode.INVALID_MESSAGE, "invalid draw payload", message.msgId());
        }
    }
