...
public class RoomValidationController {
    ...
    public ResponseEntity<?> joinRoom(@PathVariable String roomCode,
        ...
        String id = playerId == null || playerId.isBlank() ? UUID.randomUUID().toString() : playerId;
        if (id.length() > 64) return ResponseEntity.badRequest().body("invalid_player_id");

        long result = roomValidationService.joinRoom(roomCode, id);
        if (result == 1 || result == 4) {
            RoomStatusDTO status = roomValidationService.getStatus(roomCode);
            return ResponseEntity.ok(new RoomJoinResponse(id, status.members(), status.capacity(), result == 4));
        }
    ...
    public ResponseEntity<Void> leaveRoom(@PathVariable String roomId, @RequestParam String playerId) {
        return roomValidationService.leaveRoom(roomId, playerId) >= 0
