 ...
 *
 * OAuth 라이브러리를 넣지 않은 이유는 — 실제로 필요한 것은
 ...
 * 호출 하나에 허용하는 전체 시간.
 *
 * connect·read를 나눠 걸 수 없다(AbortSignal 하나가 전체를 덮는다). 그래서
 * 최악값 8초를 통짜 예산으로 잡는다 — 더 짧게 잡으면 되던 느린 로그인이
...
/**
 * `application/x-www-form-urlencoded` 인코딩(application/
