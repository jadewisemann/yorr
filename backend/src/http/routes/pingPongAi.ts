 ...
 * 잃는다(blank는 게스트, 형식 위반은 401로 가르는 이유).
 *
 * 오류 본문은 **plain-text 소문자 코드**다(프로필·auth·랭킹 REST와 같은 결).
 * 조회 REST의 JSON `{code,message}`가 아니고, 401 본문은 퀵매치의
 * `unauthorized`도 방 REST의 `invalid_guest_session`도 아닌 **`session_expired`** 다 —
 * 프론트 `shared/api/client.ts`가 그 본문을 텍스트로 읽어 대문자 코드로 매핑한다. 라우트마다 다른 이 문자열들이 계약이므로 섞지 않는다.
 */
...
export const registerPingPongAiRoutes = async (
   ...
   *
   * ① 계약상 **본문 없는 POST도 핸들러까지 들어와** `invalid_ai_result`를
   * 만들어야 한다. Fastify 기본 JSON 파서는 그 전에
