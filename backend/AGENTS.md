...
4. 손댈 자리의 **기존 테스트**를 읽는다 — 테스트가 곧 지켜야 할 동작 명세다.
...
> (`.claude/hooks/design-review-gate.mjs`): `backend/src` 변경이 설계 문서 변경
> 없이 커밋되려 하면 그 변경에 대해 **한 번** 멈추고 검토를 요구한다. 검토 후
> 문서 변경이 불필요하면 같은 커밋을 다시 실행하면 통과된다. 테스트만 바뀐
> 커밋은 멈추지 않는다.

## 계약 규칙

- **와이어 계약의 정본은 프론트엔드다.** REST 경로·응답 모양과 WebSocket
  메시지를 서버가 임의로 바꾸지 않는다(DESIGN.md 원칙 5). 바꿔야 하면
  프론트와 함께 결정하고 [PLANS.md](PLANS.md)에 절을 만들어 근거를 남긴다.
- **게임 하나 = 수직 슬라이스.** 레이어 전체를 한 번에 건드리지 않고, 게임(또는
  기능) 단위로 REST + WS + 상태 + 테스트를 끝까지 맞춰 프론트로 검증한다.
- 알려진 결함이나 비일관성을 발견하면 조용히 고치지 말고 IMPLEMENTATION_NOTES.md에
