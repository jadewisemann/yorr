...
npm run test:visual    # 시각 대조 (겉모습 before/after — 회귀 테스트가 아니다)
```
...
- UI 변경의 시각 검토도 해당 티켓의 완료 조건일 때만 수행한다. 단 **"겉모습이 안
  바뀐다"를 주장하는 변경**(토큰 회수·프리미티브 치환)은 그 주장이 곧 완료 조건이므로
  `npm run test:visual`로 대조한다 — 사용법은 [testing.md](docs/llmwiki/testing.md).
