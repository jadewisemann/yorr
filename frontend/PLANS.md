...
|---|---|---|---|
| **1** | **하드코딩 색** `white/N`·`black/N`·`text-white` | **78 → 56** | **라이트 모드** (헤어라인 22곳 회수 완료) |
...
1. **하드코딩 색 회수 (78곳 중 22곳 완료, 56곳 남음 — 테스트 제외).**
   ...
   **다음 조각은 스크림 10곳**이다. 표면(`bg-white/N`)보다 먼저인 이유: 스크림은
   사다리가 아예 없어(현재 `--ds-color-scrim` 66% 한 단) 토큰 신설이 선행돼야 하고,
   5개 값(35·45·55·65·72)이 전부 "뒤를 얼마나 가리나" 한 축이라 갈래가 단순하다.
   표면 19곳은 `surface-veil`(6%) 위 단을 몇 개 둘지가 열린 결정이라 뒤로 미룬다.
