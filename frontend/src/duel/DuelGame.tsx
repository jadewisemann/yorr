...
import { MAX_FOULS, MAX_HP, type ShotTarget, slots } from './duel'
...
export function DuelGame({ onLeaveRequest, roomId, session, snapshot }: DuelGameProps) {
  ...
  /**
   * 내가 이번 라운드에 쏜 총 — 서버 응답을 기다리지 않고 누른 순간 총이 나가게 하려고
   * 로컬로 기억한다. 라운드 번호를 함께 담는 이유는 다음 라운드가 열리는 순간 비교만으로
   * 자연히 풀리기 때문이다 — effect로 되돌리면 새 라운드의 첫 프레임에 화염이 한 번 스친다.
   */
