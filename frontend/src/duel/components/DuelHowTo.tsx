...
function Step({
  ...
  if (permission === 'denied' || permission === 'unsupported') {
    return (
      <>
        ...
        <button
          className="min-h-11 rounded-full border border-border bg-surface px-4 text-sm font-bold transition-[scale] duration-150 focus-ring pressable"
          onClick={onPractice}
          type="button"
        >
          눌러서 연습 뽑기
  ...
  return (
    <>
      ...
      <button
        className="min-h-11 rounded-full border border-brand/50 bg-brand/12 px-4 text-sm font-bold text-brand transition-[scale] duration-150 focus-ring pressable"
        onClick={onRequest}
        type="button"
      >
        휴대폰 휘두르기 켜기
