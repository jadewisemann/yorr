  ...
  return (
    ...
    >
      <div className="flex flex-col gap-1.5 pr-12">
        <span className={`${narrow ? 'text-[17px]' : 'text-[20px]'} font-bold text-landing-text`}>
          초대받은 방에 참가
        </span>
        {!narrow && (
      ...
      <div className={`flex ${narrow ? 'gap-2' : 'flex-col gap-4'}`}>
        ...
        <input
          ...
          autoComplete="off"
          className={`${narrow ? 'h-12 min-w-0 flex-1 rounded-[12px] text-[18px] tracking-[0.14em]' : 'h-18 w-full rounded-[14px] text-[30px] tracking-[0.22em]'} border bg-landing-field text-center font-mono font-bold text-landing-text placeholder:tracking-[0.18em] placeholder:text-landing-placeholder focus-visible:border-landing-text focus-visible:outline-3 focus-visible:outline-landing-accent focus-visible:outline-offset-2 ${code.length > 0 ? 'border-landing-hairline-strong' : 'border-landing-hairline'}`}
          id="room-code"
        ...
        <button
          ...
          className={`flex items-center justify-center border-0 font-bold whitespace-nowrap transition-colors duration-150 ease-out focus-visible:outline-3 focus-visible:outline-landing-accent focus-visible:outline-offset-3 ${
            narrow ? 'h-12 rounded-[12px] px-3 text-[14px]' : 'h-14 rounded-[16px] text-[17px]'
          } ${
            ready
      ...
      <span
        className={`${narrow ? 'text-[11.5px]' : 'text-[13px]'} text-landing-text-muted`}
        id="code-help"
      >
