...
## 2026-08-31 - 모바일 랭킹 팝업·캐러셀 안내 문구·낚시 카드 정리

### 랭킹 팝업은 `BottomSheet`를 쓸 수 없다

랜딩의 `GameCanvas`가 곧 `<main>`이고, `useDialogBackground`는 열릴 때 `main`에 `inert`를
건다. 그래서 캔버스 안에서 렌더되는 오버레이는 자기 자신까지 잠긴다. 랜딩 다이얼로그들이
전부 `GameCanvas` **밖**에 서 있는 이유가 이것이고, 랭킹 데이터는 `RankingTicker` 안에서
받으므로(`useFetchEffect`에 공유 캐시가 없어 밖으로 올리면 폴링이 두 벌 돈다) 팝업을 밖으로
내보내면 데이터도 함께 끌고 나가야 한다. 그래서 wide가 이미 쓰던 방식(스크림을 직접 깔고
`inert`를 쓰지 않는 `absolute` 패널)을 그대로 narrow에 확장하고, 양쪽이 공유하는
`RankingPanel`로 묶었다.

- 코드로 못 옮기는 제약: 목록 `ol`은 `button`의 자손이 될 수 없다(phrasing content만
  허용). 그래서 narrow의 탭 영역은 띠를 감싸는 버튼이 아니라 띠 위에 덮은 투명
  `absolute inset-0` 버튼이다.

### 낚시는 카탈로그에서만 내렸다

`games` 배열에서 항목만 지웠고 `GameKey`·`heroScene`의 씬·베이크 에셋(`fishing-*.webp`)은
남겨 뒀다 — 히어로 아트 파이프라인이 이 키로 굽고, 되살릴 때 배열만 되돌리면 된다.

- `e2e/mock/landing.spec.ts`의 첫 화살표 단정이 원래부터 틀려 있었다(요트에서 ArrowRight
  하면 탁구인데 라이어스 다이스를 기대했다). 마지막 게임이 바뀌면서 손대는 자리라 함께
  고쳤다.

## 2026-08-27 - 파티 탁구 판정을 대시보드로 내리며 확인한 것들

사용자 지적에서 출발했다: "주사위는 서버 권위고 실시간성도 높을 필요 없다. 탁구는 아니다."
**옳은 지적이었고 내 우선순위가 틀렸다** — 옮기기 쉬운 것(서버가 중계만 하는 연출 릴레이)을
옮겼지 필요한 것을 옮긴 게 아니었다. 결정과 경계는
[ADR-0003](docs/adr/0003-party-host-authority-pingpong.md).

### 릴레이만으로는 탁구가 해결되지 않는다

TV의 공은 서버가 준 `ball{launchedAt, pos, direction, speed}`로 그려지고, **방향이 뒤집히는
것은 서버가 새 상태를 보낼 때뿐**이다. 스윙을 링크로 옮기면 라켓만 즉시 움직이고 공은
그대로 늦어서 오히려 어긋나 보인다. 판정을 그리는 곳으로 내리는 것 말고 방법이 없었다.

### 판정 로직을 새로 쓰지 않아도 됐다

`localGame.ts`에 이미 `mode: 'duo'`와 `swingLocalGame(state, 1|2, …)`이 있었다(AI 모드의
2인 로컬 대전용). 대시보드가 그걸 그대로 돌린다. 서버 규칙(`pingPongRules.ts` 420줄)을
프론트로 옮기는 선택지도 있었지만, **이미 있는 시뮬레이션을 쓰는 쪽이 코드가 안 늘어난다.**

- 대가: 서버 규칙과 로컬 시뮬레이션이 **같은 게임의 서로 다른 두 구현**으로 남는다. 원래도
  그랬고(AI 모드), 이번 변경으로 그 차이가 파티 모드에서도 보이게 됐다.

### 서버를 실제로 멈춰야 했다

처음에는 프론트만 고치면 되는 줄 알았는데, 서버 탁구가 **마감 스케줄러로 굴러간다.**
스윙이 안 와도 공이 날아가 실점 판정을 내리고 `game.over`까지 만든다. 전적에 틀린 결과가
남으므로 파티 방에서는 예약을 걸지 않도록 고쳤다(`schedule()` 한 곳이 전부 통과하는 문이라
게이트도 거기 하나면 됐다).

### 파티 판정을 캐시로 잡은 이유, 그리고 지연 조회로 바꾼 이유

`isPartyRoom`은 Redis를 읽어야 해서 비동기다. 처음에는 `start()`에서 한 번 읽어 Map에
넣었는데, `start()`를 거치지 않는 경로(스윙이 먼저 도착, 재시작 뒤 복구)에서 캐시가 비어
서버가 판정해 버렸다. **처음 물을 때 읽고 캐시하는 지연 조회**로 바꾸고 `schedule()`을
비동기로 만들었다. 캐시가 있으면 다시 읽지 않으므로 "판 도중에 판정 주체가 바뀌지 않는다"는
불변식은 그대로다.

### 두 입력 경로를 같은 봉투로 모았다

링크로 온 스윙은 `relayedServerMessage`가, 링크가 없는 스윙은 서버가 각각
`game.ping_pong.swung`으로 만든다. **받는 쪽 코드가 하나**라 "링크가 있을 때와 없을 때가
다르게 동작한다"는 갈래가 생기지 않았다. 링크는 정확성 조건이 아니라 지연 최적화다.

- 스윙은 `dice.shake`와 달리 **한 번뿐인 신호**라 reliable 채널로 보낸다. 유실되면
  대시보드가 그 입력을 영영 모른다.

### 걸린 것

- `usePartyHostGame`을 모든 기기에서 마운트하면 컨트롤러에서도 3D 무대와 시뮬레이션이
  돌아간다. `enabled`로 막았는데, **메시지 구독까지 막아야** 했다 — 기존 화면 테스트가
  `{ send }`만 가진 최소 스텁을 쓰고 있어서 `client.onMessage`가 없다.
- 프론트 `PingPongState`는 선택 필드가 `?: T | null`인데 백엔드는 `?: T | undefined`다.
  변환에서 null·undefined를 갈라 두지 않으면 재접속 스냅샷에 "없는 필드"가 아니라
  "null 필드"가 실린다.

### 아직 못 한 것

**실기기 체감 측정.** 링크와 같은 이유로 파티 방(폰 + TV)이 필요하다. 그리고 대시보드
새로고침 시 진행 중인 랠리가 사라지는 것을 실제로 어떻게 느끼는지도 거기서 봐야 한다.

## 2026-08-27 - 컨트롤러 연결을 RTC로 옮기며 확인한 것들

"핸드폰을 컨트롤러로 쓰는 연결을 소켓에서 RTC로, 폴백은 소켓" 요청. 설계는
[controller-link.md](docs/llmwiki/controller-link.md)와
[ADR-0002](docs/adr/0002-controller-link-signaling.md)로 승격했고, 여기에는 판단 근거와
확인 과정만 남긴다.

### 옮길 수 있는 메시지가 둘뿐이었다 — 세어 보고 알았다

컨트롤러가 보내는 메시지 9종을 전수 확인한 결과, **서버가 판정·저장하지 않고 그대로
중계만 하는 것은 `dice.shake`·`dice.throw` 둘뿐**이다. 나머지는 서버가 주사위를 굴리거나
(`dice.roll`), 라운드 상태에 저장하거나(`dice.hold`의 `held`는 재접속 스냅샷에 실린다),
`clientTs`로 되감아 판정한다(`ping_pong.swing`). 서버는 WebSocket만 말하므로 이들은
링크로 갈 수 없다 — "컨트롤러 연결을 통째로 RTC로"는 **원리적으로 불가능**하고, 줄일 수
있는 것은 연출 릴레이의 서버 왕복 한 홉이다. 이 판정을 DESIGN.md 원칙 1에 한 줄로
못박았다.

- 그래서 **탁구·석양이 진다는 링크를 쓰지 않는다.** 두 게임은 지연을 전송이 아니라 판정
  시각 보정(`clientTs` 되감기 120ms, 반응 시간 자가 측정)으로 이미 다루고 있다.
- 석양이 진다에 "연출용으로만 곁들여 보내기"를 검토했다가 접었다 — 무대 불변식 3(상대
  총알·기록은 판정 후에만)을 정면으로 깨고, 밸런스 페널티가 **전송을 늦춰서** 걸리는
  구조라 빠른 경로가 오히려 해롭다.

### 처음 설계가 음성 삭제로 무효가 됐다

첫 판은 **`voice.signal`에 갈래를 얹는 것**이었다. 그 릴레이가 이미 방 스코프 불투명
유니캐스트라 백엔드 변경이 0이었기 때문이다. 그런데 그 사이 음성이 텍스트 채팅으로
교체되면서 `voice.*`와 `GET /voice/ice`가 통째로 사라졌다.

- **`chat.*`은 대체가 안 된다.** 방 전체 브로드캐스트이고 글자 수·도배 한도가 걸려 있어
  SDP를 실을 수 없다. 그래서 기각했던 대안(`ctrl.*` 새 타입)이 유일한 길이 됐다.
- 결과적으로 더 깔끔해졌다. 릴레이를 공유하던 동안에는 **양쪽이 서로의 시그널을 걸러야**
  했고(한쪽만 걸러도 통화나 링크 중 하나가 엉킨다), 그 결합이 없어졌다.
- 서버 코드는 지워진 `voice.ts`의 유니캐스트 부분과 같다
  (`git log -p -- backend/src/ws/voice.ts`). 테스트도 그쪽 릴레이 케이스를 옮겼다.

### 대시보드가 offer를 만들어야 한다

**서버가 파티 대시보드를 방 스냅샷의 플레이어 명단에 넣지 않으므로 폰은 대시보드의
playerId를 모른다.** 반대로 대시보드는 스냅샷 `players`에서 폰들의 id를 본다. 확인해 보니
WS 레지스트리(`backend/src/ws/registry.ts`)에는 대시보드도 들어 있어서 **playerId만 알면
릴레이가 닿는다** — 그래서 협상 방향을 고정하는 것으로 해결됐고, 역할이 비대칭이라
"id가 작은 쪽이 offer" 같은 대칭 규칙이 필요 없다.

### TURN을 붙이지 않기로 한 근거

중계 경로는 `폰 → TURN → TV`이고, 폴백인 `폰 → 앱서버 → TV`와 **홉 수가 같다.** 같은
호스트에 띄우면 완전히 동일하다. 즉 대역폭·coturn 운영·포트 개방을 지불하고 얻는 것이
없다. 음성과 갈리는 지점이 여기다 — 음성에는 폴백이 없어서 TURN 없이는 기능 자체가 실패했다.

STUN은 유지하되 상수로 박았다. 트래픽이 지나가지 않아 비용이 없고, 손님 폰이 LTE인 경우를
덮는다. 같은 랜에서는 STUN이 없어도 host 후보로 붙지만 있어도 느려지지 않는다(host 후보
쌍이 먼저 검사되고 srflx 수집은 병렬이다). 같은 랜인데 안 붙는 경우는 mDNS 난독화를
못 푸는 망(멀티캐스트 차단·단말 격리)인데, 그때는 TURN을 붙여도 마찬가지다.

### 배선에서 걸린 것 셋

- **`realtime/`은 방 종류를 모른다.** 링크의 존재 조건은 "파티 방인가"이고 그 판정은
  `room/partyControllerStorage`에 있다. `realtime/ → room/`은 되돌아가는 import라 역할
  판정을 `room/model/useControllerLinkRole.ts`로 빼고 `App`이 prop으로 내린다.
- **prop 이름을 `role`로 두면 biome가 ARIA role로 읽는다**(`lint/a11y/useValidAriaRole`).
  `linkRole`로 고쳤다. JSX 컴포넌트에 `role` prop을 만들면 안 되는 자리다.
- **RTT를 컨텍스트 값에 담으면 게임 중 2초마다 소비자의 effect가 재실행된다**(측정 주기가
  곧 값 변경이다). `rttMs`를 값이 아니라 함수로 노출하고 컨텍스트 값은 `status`가 바뀔
  때만 새로 만든다. 이 성질을 테스트로 고정했다.
- 곁따라 사라진 구멍: ICE 발급 REST를 기다리던 배선은 응답이 오지 않는 망에서 링크가
  영영 만들어지지 않았다(타임아웃이 없었다). 상수가 되어 동기 생성으로 바뀌며 없어졌다.

### 아직 못 한 것

**실기기 지연 측정.** 절감 근거는 구조적인 것뿐이다(홉 하나 + 서버 처리 + TCP
head-of-line blocking). 재기 위해 RTT 프로브를 넣었지만(`useControllerLink().rttMs()`)
화면에 노출하지 않았고, 파티 방(폰 + TV)이 필요한 검증이라 열어 둔다.

## 2026-08-27 - 음성 → 텍스트 채팅 전환에서 걸린 것들

계획·결정은 [PLANS.md](PLANS.md)의 같은 이름 절, 결과 구조는
[chat.md](docs/llmwiki/chat.md)에 있다. 여기에는 **코드에 흔적을 남기지 못한 함정**만
적는다.

- **안 읽은 수를 인덱스로 세면 안 된다.** 처음에는 "마지막으로 본 줄의 인덱스"로
  잡았는데, 목록이 `CHAT_HISTORY_LIMIT`(50)을 넘어 앞줄이 잘려 나가면 기준선이 함께
  밀려서 **읽지 않은 말이 읽음으로 바뀐다.** 「남이 보낸 말의 누적 개수」로 세면 잘림과
  무관해진다. 코드에는 결론만 주석으로 남았다.
- **방이 바뀔 때의 초기화는 effect가 아니라 렌더 중 조정이어야 한다.** effect로
  지우면 새 방의 첫 프레임에 옛 방 대화가 한 번 그려진다. 부수적으로 biome의
  `useExhaustiveDependencies`도 사라진다 — effect 본문이 읽지 않는 `you`를 의존성에
  넣어야 했기 때문이다.
- **jsdom에는 `scrollIntoView`가 없다.** 마지막 줄 따라가기가 테스트 전부를
  `TypeError`로 죽였다. 프로덕션 코드에서 옵셔널 호출(`?.()`)로 넘긴다 — 화면 밖
  환경에서 못 따라가는 것은 문제가 아니다.
- **`matches`만 있는 `matchMedia` 스텁은 반쪽이다.** `LobbyPage.test.tsx`가 그렇게
  두고 있었는데, 대기실이 미디어 질의를 구독하기 시작하자(채팅 창 레이아웃 분기)
  `addEventListener is not a function`으로 17건이 한꺼번에 깨졌다. 스텁에 리스너 두
  개를 넣어 고쳤다 — 컴포넌트 쪽 문제가 아니었다.
- **mock 백엔드의 strict 모드는 조용히 삼켜진다.** `FakeRealtimeClient`는 모르는
  타입에 던지고, `useRoomChat`의 `send`는 그 예외를 잡는다. 그래서 에코 핸들러를
  넣기 전에는 mock으로 도는 화면에서 채팅이 **아무 오류 없이 아무 일도 안 일어났다.**
- **`RATE_LIMITED`가 처음으로 실제 쓰였다.** 계약에는 있었지만 전송 경로가 없던
  코드다(백엔드 `protocol.ts` 주석이 그 사실을 적어 두고 있었다). 도배 판정에서
  거절한 시도를 기록하지 않는 것이 중요하다 — 세면 도배하는 쪽이 창을 계속 밀어
  영구히 막힌다.
