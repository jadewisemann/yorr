...
## 2026-08-21 - 랜딩의 라이트 편입 — 크롬은 뒤집고 무대는 남긴다

2026-08-18 노트의 "랜딩 팔레트는 테마 제외가 의도" 결정을 사용자가 뒤집었다
(라이트에서 랜딩이 다크로 남는 것이 이상해 보인다). 편입 방식은 반반이다:

- **크롬(배경·티커·헤더·프로그레스·배너·코드 필드·그림자)은 라이트 블록이 덮는다.**
  값은 본편 사다리를 그대로 따랐다 — 글자 4단은 content 계열(#0b0c0f·#16171b·
  #55565c·#616269), accent-text는 라이트 brand-soft(#b3231f), 헤어라인은 공유
  사다리 참조라 손대지 않아도 이미 뒤집혀 있었다.
- **히어로 카드 무대는 어둡게 남는다.** 3D 피사체가 아이보리(주사위 #f4f1e8)라 밝은
  카드에서는 실루엣이 사라지고, 카드가 미리 보여주는 게임 무대 자체가 라이트에서도
  어둡다(pp·duel과 같은 근거). 카드 바닥·광택·스크림은 라이트 블록이 안 덮고,
  그 위 글자·헤어라인·필 바닥은 `[data-theme="light"] .landing-stage`가 다크 값으로
  되돌린다. 스코프는 `LandingHeroCarousel`의 `<section>` 하나 — 가운데 카드·이웃
  카드·화살표가 전부 그 안이라 클래스 한 곳으로 끝난다.
- **함정 둘.** ① 카드 겉 그림자(shadow-card·card-quiet)는 라이트 페이지에 떨어지므로
  무대가 되돌리면 안 된다(라이트 값 30·26%로 덮고 stage 재선언 목록에서 뺐다).
  card-inset은 카드 안쪽 비네팅이라 반대로 안 덮는다. ② 무대 위 CTA의 포커스 링
  (`outline-focus`)은 시스템 focus를 참조해 라이트에서 #16171b가 된다 — 어두운 카드
  위에서 안 보이므로 stage가 `--ds-color-focus`도 #f7f7f5로 되돌린다.
