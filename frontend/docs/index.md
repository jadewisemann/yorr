# YORR 프론트엔드 문서 — LLM 위키

> 기준일: 2026-08-13

이 디렉터리는 프론트엔드 코드베이스가 **실제로 어떻게 동작하는지**를 설명하는 위키다.
사람과 LLM(에이전트) 모두를 독자로 삼되, 구조는 LLM 우선이다 — 인덱스 하나에서 출발해
필요한 페이지만 골라 읽고, 페이지마다 SSOT 코드 경로가 달려 있어 검증 가능하다.

**에이전트는 이 문서 하나만 인덱스로 삼는다.** 아래 표에서 필요한 파일만 골라 열고,
관련 없는 문서는 열지 않는다. 읽기 원칙은 [`../CLAUDE.md`](../CLAUDE.md)를 따른다.
