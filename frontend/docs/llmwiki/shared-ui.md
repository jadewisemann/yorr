...
| `AudioPopover` | 마스터 음소거 + 음악/효과 슬라이더. 마이크 행은 음성 채팅과 함께 사라졌다([chat.md](./chat.md)) |
...
- `audio/` — iOS 볼륨 우회(`elementVolume`), BGM(`soundtrack`), 효과음, 음소거 영속화,
  첫 제스처 언락. iOS Safari는 `HTMLMediaElement.volume` 대입을 조용히 무시하고 **기능
  탐지로도 속으므로**(대입한 값을 그대로 되읽어 준다) 항상 `createMediaElementSource →
  GainNode`로 흘린다. AudioContext는 모든 `pointerdown/touchend/keydown`에서
  `resume()`한다 — 한 번만 깨우면 다시 잠긴 뒤 영영 무음이다. 실측 근거는
  [code-rationale.md](./code-rationale.md)의 `elementVolume`·`handVoice` 항목.
