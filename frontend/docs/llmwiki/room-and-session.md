...
- 초대 UI는 인라인 카드에서 **말풍선 팝오버**로 이전 — 320×568에서 카드가
  ...
  항시 노출할 가치가 없다.
- **채팅은 그 반대 판정이다** — 대기실에서 대화는 계속하는 일이라 늘 보이게 둔다. 다만
  초대 카드가 겪은 세로 압박을 되풀이하지 않도록 **흐름에서 빼냈다**: 좁은 화면에서는
  `ChatDock`이 본문 위에 떠 있고(접히면 최근 3줄), 넓은 화면에서만 `ChatPanel`이 오른쪽
  열을 차지한다. 자세한 갈래는 [chat.md](chat.md).
- 대기실 본문(컨트롤러 안내·봇 패널·참가 인원 줄·참가자 목록)은 **하나의 스크롤 영역**이고
  헤더·시작 버튼은 그 밖에서 자리를 지킨다. 목록만 따로 굴리게 두면 시작 버튼이 화면 밖으로
  밀려난다. 스크롤 영역의 위쪽 `pt`(4.75rem)는 접힌 도크가 앉을 자리다.
