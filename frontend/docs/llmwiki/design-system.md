...
inverse 쌍 · brand의 위아래 단 · 상태색 3종 · 그림자 2종 · **랜딩 전체**(배경·
패널·글자·필드·그림자에 히어로 카드의 바닥·크라운·스크림·고스트까지 — 게임과
달리 랜딩은 화면 전체가 테마를 따른다. 크라운·스크림은 뒤가 밝은 카드이므로 흰
막으로 뒤집는다). 카드 위 3D 피사체(heroScene)는 physics 토큰·고정 색이라 테마를
안 타므로, 라이트 카드 바닥은 가장자리로 갈수록 가라앉는 그라디언트로 아이보리
피사체의 실루엣을 지킨다.
…그리고 **요트 3D 트레이**(`physics-tray`·`physics-tray-shadow`·`physics-rail`·
`physics-slot`·`physics-ground`). 탁구·결투와 갈리는 기준은 "게임인가"가 아니라
**"화면을 혼자 쓰는 무대인가"** 다: 탁구 코트·결투장은 뷰포트를 꽉 채우고 그 위 글자가
`game-content-*`(흰색 알파)를 쓰지만, 요트 트레이는 앱 크롬(헤더·점수표) 사이에 끼워진
판이고 그 안의 라벨·칩이 `content-faint`·`surface` 같은 **앱 토큰을 그대로 쓴다**.
검게 두면 라이트에서 그 글자들이 검정 위 검정이 된다 — "요트만 라이트가 안 먹는다"는
증상의 정체가 이것이었다.

**안 덮는 것(의도)**: 게임 팔레트(`pp-*`·`duel-*`) — 세계관이 다르다, 탁구 코트는
라이트에서도 어두운 무대다.
주사위 몸통과 눈(`physics-die`·`physics-pip`) — 실물이라 테마와 무관하게 상아색·검정이다
(바닥이 되는 트레이만 갈린다). 스크림 — 뒤를 가리는 일은 양쪽
모두 검정이 한다. `brand` 자체 — 두 테마에서 같은 레드다(랜딩 `accent` 3종과
`accent-line` 포함).
...
빠뜨리면 실패). 그래서 **3D는 physics 토큰만 읽을 수 있다.**

**읽는 시점**도 규칙이다: `World.ts`가 `documentElement`에 `MutationObserver`를 걸어
`data-theme`이 바뀔 때마다 `syncAppearance`로 **다시 읽는다**. 그래서 트레이 팔레트를
라이트에서 뒤집어도 컴포넌트는 손댈 것이 없다 — 토큰만 덮으면 3D가 따라온다.

3D 재질값을 고를 때 두 가지가 CSS와 다르다.

- **화면색 ≠ 토큰값.** 렌더러가 ACESFilmic 톤매핑(노출 1.08)을 거치므로 중간톤은
  밝아지고 밝은 쪽은 압축된다(실측 `#b9b7b0`→`#c7c6c2`, `#ffffff`→`#e5e5e5`).
  라이트 트레이 값은 **그렇게 나오도록 역산해 넣은 값**이라 눈으로 고를 수 없다 —
  바꿀 때는 화면을 찍어 확인한다.
- **알파가 무시된다.** `THREE.Color.set`은 `rgb(r g b / a%)`의 알파를 읽지 못하고
  재질에 transparent도 걸려 있지 않다. `--ds-color-physics-slot`의 다크값이
  `rgb(255 255 255 / 14%)`인데 화면에는 **불투명 흰 막대**로 나오는 것이 그 결과다.
