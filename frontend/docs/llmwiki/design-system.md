...
inverse 쌍 · brand의 위아래 단 · 상태색 3종 · 그림자 2종 · **랜딩 크롬**(배경·
패널·글자·필드·그림자 — 히어로 카드 무대는 제외).
**안 덮는 것(의도)**: 게임 팔레트(`pp-*`·`duel-*`) — 세계관이 다르다, 탁구 코트는
라이트에서도 어두운 무대다. 랜딩 히어로 카드의 바닥·광택·스크림
(`--ds-landing-card*`·`ghost`·`accent-line`)도 같은 이유 — 카드는 게임 무대의
미리보기고 3D 피사체(아이보리 주사위)가 어두운 바닥을 전제한다. 그래서
`LandingHeroCarousel`의 `<section>`이 `.landing-stage`를 달고, 라이트에서 그 서브트리의
글자·헤어라인·필 바닥 변수만 다크 값으로 되돌린다(tokens.css 라이트 블록 끝) —
컴포넌트는 여전히 테마를 모르고, "이 서브트리는 무대다"만 선언한다.
주사위(`physics-*`) — 실물이라 테마와 무관하다. 스크림 — 뒤를 가리는 일은 양쪽
모두 검정이 한다. `brand` 자체 — 두 테마에서 같은 레드다(랜딩 `accent` 3종 포함).
