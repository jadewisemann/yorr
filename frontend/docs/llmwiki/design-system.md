...
> ⚠ **`text-brand`(민 brand)는 라이트에서 canvas 위 3.54:1로 미달**이다(다크는 4.71).
> 빨간 글자의 자리는 `brand-soft`다 — 라이트를 사용자에게 켜기 전에 `text-brand`
> 3곳을 옮겨야 한다. 값과 근거는 `tokens.css`의 라이트 블록 주석에 있다.
