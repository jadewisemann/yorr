...
- **전체 순위 팝업은 두 레이아웃이 같은 `RankingPanel`을 쓴다** — wide는 "전체 보기" 버튼,
  narrow는 띠 전체를 덮은 투명 버튼이 앵커다(목록 `ol`은 `button` 안에 들어갈 수 없다).
  이 패널은 `useDialogBackground`를 쓰지 않는다: 랜딩 캔버스가 곧 `<main>`이라 `inert`를
  걸면 캔버스 안에 그려진 패널이 자기 자신을 잠근다. 그래서 스크림도 직접 깐다.
