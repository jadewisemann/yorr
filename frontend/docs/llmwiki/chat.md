...
| `ChatOverlay.tsx` | 좁은 화면 게임판 위에 겹치는 최근 대화. 누르면 입력용 시트를 연다 |
| `ChatDialog.tsx` | 대화 **창**(바텀시트) + 여는 버튼 보조 요소. 좁은 화면 게임판의 입력 자리 |
...
- **`useChat()`은 provider 밖에서 던지지 않고 `NO_CHAT`으로 강등한다.**
  ...
  게임이 돌아가므로 심각도가 다르다.)
- **대화는 어느 화면에서도 상주한다.** 창을 열어야 보이는 대화는 방에 함께 있다는
  감각을 만들지 못한다. 다만 자리가 없는 곳이 있어 형태가 셋으로 갈린다.

  | 자리 | 형태 | 이유 |
  |---|---|---|
  | 대기실 | `ChatPanel` — 좁은 화면은 고정 높이, 넓은 화면은 오른쪽 열 | 대화가 주된 일거리다 |
  | 넓은 화면 게임판 | `ChatPanel` — 점수표 아래 고정 높이 | 오른쪽 열에 자리가 있다 |
  | 좁은 화면 게임판 | `ChatOverlay`(최근 3줄) + 누르면 `ChatDialog` 시트 | 굴리기 푸터·기록 패널·리액션 독이 이미 화면 아래를 층층이 쓴다 |

  두 형태가 나뉘는 자리는 **껍데기와 높이 정책뿐**이고, 읽음 처리·마지막 줄 따라가기·거절
  처리는 `ChatBody` 하나에 있다.
- **보이면 읽음이다.** 상주 패널에도, 판 위 오버레이에도 안 읽은 수가 없다 — 대화가 그대로
  보이는데 배지가 남아 있으면 거짓말이 된다. 안 읽은 수가 실제로 쓰이는 곳은 좁은 화면
  게임판의 헤더 버튼 하나뿐이고, 그마저 오버레이가 읽음으로 만들어 준다.
