...
## 물리 주사위 월드의 구성 (2026-09-01)

`physics-dice/World.ts`는 760줄에서 506줄이 됐다. 밖으로 나간 것은 **한 프레임 안에서
끝나는 계산**들이다.

| 모듈 | 하는 일 |
|---|---|
| `bowlMotion.ts` | 흔들기 · 기울여 쏟기 · 화면 밖으로 빠지기 · 컨트롤러 펄스 임펄스 |
| `diceFlight.ts` | 방출 임펄스 · 궤적 계획과 재생 · 착지 충격 감지 |
| `diceSeating.ts` | 굴림 시작 배치 · 정사영 절두체 |
| `resultAlignment.ts` | 결과 줄 정렬 · 킵 자리 이동 |
| `resizeSettler.ts` | 크기 변화가 가라앉은 뒤에 반영 |

**한 가지 함정을 남긴다.** 쏟는 프레임에서 그릇의 물리 몸체는 주사위를 놓은 뒤로는
움직이면 안 된다(`pourBowlFrame`의 `if (!diceReleased && !releaseDue)`). 원래 코드는
조기 반환으로 그것을 표현하고 있었는데, 함수로 뽑으면서 조건을 놓치면 막 튀어나온
주사위를 그릇이 다시 밀어낸다.
