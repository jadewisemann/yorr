...
- 서버가 방·게임 상태의 최종 권위자. 방 텍스트 채팅도 같은 WS로 중계된다(서버 저장 없음).
...
| 04 | `davinci` | 다빈치 코드 | 2–4 | 화면 탭 | live (`DAVINCI_CODE`) |
| 05 | `liars` | 라이어스 다이스 | 2–6 | 화면 탭 | 준비 중 |

낚시(`fishing`)는 카탈로그에서 내렸다 — 히어로 아트(`heroScene`·베이크 에셋)와 `GameKey`
항목은 남아 있어, 되살리려면 `games` 배열에 항목만 되돌리면 된다.
...
- 단위 테스트가 부하 상태에서 간헐 실패하는 이슈(실측)와 그 대응은
