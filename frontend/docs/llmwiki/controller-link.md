...
| `game.ping_pong.swing` | ✅ | **파티 모드에서는 서버가 탁구를 판정하지 않는다**([ADR-0003](../adr/0003-party-host-authority-pingpong.md)) — 스윙이 가야 할 곳이 판정하는 큰 화면이다. 링크가 없으면 서버가 받아 `game.ping_pong.swung`으로 대시보드에 전달한다 |
...
즉 **석양이 진다는 링크를 쓰지 않는다.** 판정도 서버지만, 연출용으로 곁들여 보내는 것조차
무대 불변식을 깨기 때문이다.

탁구는 성격이 다르다. 나머지가 "서버가 중계만 하니 옮길 수 있다"인 반면, 탁구는 **판정을
큰 화면으로 내렸기 때문에** 옮길 수 있게 된 것이다([ADR-0003](../adr/0003-party-host-authority-pingpong.md)).
그래서 스윙은 유실되면 안 되는 신호이고 reliable 채널로 간다.
