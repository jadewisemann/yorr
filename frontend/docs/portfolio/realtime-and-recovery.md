...
| 코드 | `src/realtime/` · `src/app/RealtimeSync.tsx` · `src/room/domain/sessionFsm.ts` — 상세는 [위키: realtime](../llmwiki/realtime.md) · [room-and-session](../llmwiki/room-and-session.md) |
...
- "모바일 WebSocket 세션의 수명주기를 파생 상태 FSM으로 설계하고, heartbeat 생존
  판정부터 sessionToken 복원까지 서버 측을 직접 구현해 끝단 간 복구를 완성"
