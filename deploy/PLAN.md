...
| 호스트 계정은 **`opc`**(Oracle Linux 계열) — 추적 파일의 `User=ubuntu`는 이 호스트에 맞지 않는다 | SSH 접속(2026-08-23) |
...
- **가설 1 — `git pull --ff-only`가 영구 실패하고 있다.** `deploy/systemd/`의 두
  ...
  **동시에 죽는다.** 위 세 번째 명령이 확인한다.
  → **2026-08-23: 확률이 올라갔다.** 호스트 계정이 `opc`인데 추적 파일에는
  `User=ubuntu`가 박혀 있다(위 표). 즉 "고쳐야 한다"는 압력이 가정이 아니라 실재했다.
  `--install`을 썼다면 치환되어 무사하지만, 문서를 그대로 따라 손으로 고쳤다면 막혀 있다.
